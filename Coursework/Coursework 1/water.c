...
Date Work Commenced: Tue 2 November 2021
...
// YOU WILL ALSO USE THIS SECTION TO INITIALISE ANY VARAIABLES THAT REQUIRE PROPER INITIALISATION
  ...
  double unitprice;
  double dometicCounter;
  double domesticBillTally;
...
switch (CustomerType){
    ...
    QTotalFreshWater += QWaterConsumption;
        int a;
        for(a = 1; a <= QWaterConsumption; a = a + 1){
            if (a <= 5){
                QFreshWaterCharge += 0.2;
            }
            if (a > 5 && a <= 12){
                QFreshWaterCharge += 0.35;
            }
            if (a > 12 && a <= 25){
                QFreshWaterCharge += 0.50;
            }
            if (a > 25 && a <= 40){
                QFreshWaterCharge += 0.75;
            }
            if (a > 40){
                QFreshWaterCharge += 2.5;
            }
        }
        ...
        domesticBillTally += QTotalCharge;
    ...
    case 2:
        QTotalFreshWater += QWaterConsumption;
        unitprice = 2.50;
        QFreshWaterCharge = unitprice * QWaterConsumption;
...
QAverageDomesticBill = domesticBillTally/dometicCounter;
QCost = QTotalFreshWater * 1;
// Profit = Revenue - Cost
